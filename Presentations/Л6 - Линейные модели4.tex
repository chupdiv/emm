...
\begin{frame}{Пример}
	...
	\only<3->{
		...
		\coordinate (B) at (8.35,6);	
		\coordinate (C) at (8.35,5.1);	
		...
		\only<5->{
			\filldraw[blue] (B)+(0.4,-0.2) node{$\bf-15$};
			\draw[blue] (D)+(0.3,-0.3) node{$\bf-15$};
			\draw[blue] (A)+(0.5,-0.3) node[rounded corners,fill=white,opacity=0.8]{\small$\bf+15$};
			\draw[blue] (C)+(0.2,-0.3) node{$\bf+15$};
		}
